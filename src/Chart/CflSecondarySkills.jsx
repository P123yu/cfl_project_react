...
const CflSecondarySkills = ({ empId, quarter }) => {
  ...
  useEffect(() => {
    if (empId) {
      axios
        ...
        .then((res) => {
          const secondarySkills = res.data.secondarySkills || "";
          const parts = secondarySkills.split(",").map((part) => part.trim());
          ...
          setCflSkills({ ...res.data, secondarySkills }); // Update cflSkills state
        })
        .catch((error) => {
  ...
  const labels = cflSkills.secondarySkills
    ? cflSkills.secondarySkills
        .split(",")
        .map((part) => part.trim())
        .filter(
          (part) =>
...
export default CflSecondarySkills;
